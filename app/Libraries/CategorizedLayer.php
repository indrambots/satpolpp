...
class CategorizedLayer
{
    ...
    public function create( $map_name,$data,$jumlah_data, $type,$color,$column_name,$label_show ) {
        ...
        //return $sld_name;
        $array = array('name' =>$sld_name, 'file' => storage_path('app').'/'.$this->sld_path.$file_name);
        return $array;
    }
    ...
    //LINE SYMBOLIZER
    function createLineSymbolizer( $color, $width, $kolom, $column_name ){
    ...
    //POLYGON SYMBOLIZER
    function createPolygonSymbolizer( $fcolor, $opacity, $scolor, $width, $kolom, $column_name ) {
        $polygon  = '<Rule>';
    ...
    //TEXT SYMBOLIZER
    function createTextSymbolizer( $attr, $size, $color, $offsetX, $offsetY, $PointPlacement = true) {
        $text = "<TextSymbolizer>";
