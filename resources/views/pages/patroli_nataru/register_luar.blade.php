    ...
    <div class="col-12 col-lg-12 col-xl-12 mb-2">
        <div class="card card-custom wave wave-animate-fast wave-primary">
            <div class="card-body text-center">
                <h1>
                    REGISTERASI PESERTA PATROLI NATARU SATLINMAS DESA
                </h1>
            </div>
        </div>
    </div>
...
<div class="row">
    <div class="col-lg-12">
        <div class="card card-custom gutter-b example example-compact">
            ...
            <!--begin::Form-->
            <form class="form" method="POST" action="{{ url('patroli/save-luar') }}">
            {{ csrf_field() }}
                <div class="card-body">
                    ...
                    <div class="form-group">
                        <label class="mr-20">
                         ...
                         <select style="width:90%" class="select2  form-control" id="prov" name="prov"  required >
                            <option value="">--Pilih Provinsi--</option>
                            @foreach($prov as $k)
                                <option value="{{ $k->id_prov }}">{{ $k->nm_prov }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
...
<script type="text/javascript">
    ...
    $.ajax({
      method:'POST',
      url:'{{ url("patroli/filter-master-prov") }}',
      data:{
        prov: $(this).val(),
      ...
      success:function(data){
        console.log(data);
        $('#kota').html(data.view_kota);
        $('#kecamatan').html(data.view_kec);
        $('#kelurahan').html(data.view_kel);
      }
    })
  })
    ...
    $.ajax({
      method:'POST',
      url:'{{ url("patroli/filter-master-kota") }}',
      data:{
        prov: $('#prov').val(),
        kota: $(this).val(),
        '_token': $('input[name=_token]').val()
    ...
    $.ajax({
      method:'POST',
      url:'{{ url("patroli/filter-master-kec") }}',
      data:{
        prov: $('#prov').val(),
        kota: $('#kota').val(),
        kecamatan: $(this).val(),
